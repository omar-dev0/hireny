...
class LocalOrgData implements OrgDataInterface {
  final List<OrgEntity> fixedOrgsData = [
    OrgEntity(
      orgID: 1,
      orgPresenterName: 'Andrew Walter',
      orgEmail: 'hr@eliteworkforce.com',
      joinDate: '09-12-2010',
      status: OrgStatus.pending,
    ),
    OrgEntity(
      orgID: 2,
      orgPresenterName: 'Lina Robert',
      orgEmail: 'support@globalcareers.com',
      joinDate: '22-02-2013',
      status: OrgStatus.pending,
    ),
    OrgEntity(
      orgID: 3,
      orgPresenterName: 'Sophia James',
      orgEmail: 'support@globalcareers.com',
      joinDate: '22-02-2013',
  ...
  LocalOrgData() {
    orgPendingData =
        fixedOrgsData.where((org) => org.status == OrgStatus.pending).toList();
  }
  ...
  Future<List<OrgResponse>> getOrg() async {
    ...
    for (var org in orgPendingData) {
      print(
        '-> ID: ${org.orgID}, Presenter: ${org.orgPresenterName}, Email: ${org.orgEmail}, Date: ${org.joinDate}',
      );
    }
    return orgPendingData
        .map(
          (org) => OrgResponse(
            orgID: org.orgID,
  ...
  Future<OrgResponse> verifyOrg(OrgEntity orgData, bool orgState) async {
    ...
    fixedOrgsData[index].status =
        orgState ? OrgStatus.approved : OrgStatus.rejected;
