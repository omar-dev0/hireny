...
import 'package:hireny/features/show_courses/presentation/ui/manager/course_seeker_state.dart';
...
class _ShowCoursesUiState extends State<ShowCoursesUi> {
  late SeekerCoursesCubit cubit;

  @override
  void initState() {
    super.initState();
    cubit = context.read<SeekerCoursesCubit>();
    cubit.loadCourses();
  }
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            SizedBox(height: 8),
            Expanded(
              child: BlocBuilder<SeekerCoursesCubit, SeekerCoursesSate>(
                builder: (context, state) {
                  if (state is LoadingSeekerCourse) {
                    return Center(child: CircularProgressIndicator());
                  } else if (state is ErrorSeekerCourse) {
                    return Center(
                      child: Text(state.message ?? "An error occurred"),
                    );
                  } else if (state is SuccessSeekerCourse) {
                    return cubit.courses.isEmpty
                        ? Center(
                          child: Text(
                            "You haven't added any courses yet  Tap the '+' button to begin",
                            style: Theme.of(
                              context,
